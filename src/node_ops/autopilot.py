...
# Network_Improvement: nodes are sampled from a uniform distribution of the nodes which are badly connected
# richness: nodes with high liquidity are taken and it is sampled from a uniform distribution of those
the program needs the following dependencies:
...
class Autopilot:
    ...
    def __init__(self):
        ...
        """ This init will only be successful if we're connected to another node"""
    ...
    def __load_edges(self):
        ...
        if len(self.G.nodes) == 0:
            self.__logger.info("cannot download channels if nodes do not exist. Try downloading the node list...")
            if not self.__load_nodes():
                self.__logger.info("Stop trying to download channels")
                return False
    ...
    def __generate_rich_nodes(self, k=3, percentile=0.5):
        """
            generates a set of nodes that are very well connected (such nodes with low degree will be preferred)
        """
        if k < 3:
            k = 3
        ...
        for n, value in sorted(rich_nodes.items(), key=itemgetter(1), reverse=True):
            cumsum +=value
            self.__logger.debug("RICH_NODES: node {} has {} neighbors and a balance of {}"
                                .format(n, value, len(list(self.G.neighbors(n)))))
            candidates.append(n)
    ...
    def __generate_central_nodes(self, k=3):
        """
            ...
            #FIXME: not sure if this is even helpful. I imagine that one would not want to connect to those.
             it is good since it keeps diameter small but it is bad as it makes the network depend on those
        """
        if k < 3:
            k = 3
        ...
        self.__logger.info("CENTRAL_NODES: Generated top {} central nodes " +
                           "(according to betweeness centrality)".format(len(res)))
    ...
    def __generate_random_nodes(self, k=3):
        ...
        self.__logger.info("RANDOM_NODES: try to generate a set of {} nodes " +
                           "sampled with uniform distribution".format(k))

        return random.sample(self.G.nodes(), k)

    def __generate_connectivity_increasing_candidates(self, k=3):
        """
        ...
        self.__logger.info("IMPROVE_NETWORK: Try to seek {} nodes which are currently bad connected".format(k))
        ...
        self.__logger.info("IMPROVE_NETWORK: Found {} candidates which are currently bad connected".format(len(result)))
        if len(result) <= k:
            return list(result)
        self.__logger.info("IMPROVE_NETWORK: Sample {} items from the candidates as was requested".format(k))
        tmp = list(result)
        random.shuffle(tmp)
        return tmp[0:k]
    ...
    def get_candidates(self,k=10):
        sub_k = math.ceil(k/4)
        self.__logger.info("GENERATE CANDIDATES: Try to generate up to {} nodes with 4 strategies: " +
                           "(random, central, network Improvement, liquidity)".format(k))

        candidates = set()
        """ res = self.__generate_random_nodes(sub_k)
        candidates=candidates.union(set(res))"""
        res = self.__generate_central_nodes(k)
        candidates=candidates.union(set(res))
        """res = self.__generate_rich_nodes(sub_k)
        candidates=candidates.union(set(res))
        res = self.__generate_connectivity_increasing_candidates(sub_k)
        candidates=candidates.union(set(res)) 
        ...
        self.__logger.info("GENERATE CANDIDATES: Found {} " +
                           "nodes with which channel creation is suggested".format(len(candidates)))
        return candidates

    def __calculate_statistics(self, candidates):
        """
        computes statistics of the candidate set about connectivity, wealth and returns a probability density function
         (pdf) which
        encodes which percentage of the funds should be used for each channel with each candidate node

        the pdf is proportional to the average balance of each candidate and smoothed with a uniform distribution
        currently the
        ...
        for k,v in pdf.items():
            ...
            print("{:12.2f}  ".format(100*v), "{:12.2f} ".format(100*(w * v + (1-w)/len(candidates))), "{:10}" +
                    " {:10}     ".format( capacity, len(neighbors)), name)
    ...
    def __calculate_proposed_channel_capacities(self,pdf, candidates, balance = 1000000,):
        ...
        self.__logger.info("Need at least a balance of {} satoshi to open {} channels"
                           .format(needed_total_balance, len(candidates)))
        while needed_total_balance > balance:
            min_val = min(pdf.values())
            k = [k for k, v in pdf.items() if v == min_val][0]
            self.__logger.info("Not enough balance to open {} channels. " +
                             "Remove node: {} and rebalance pdf for channel balances".format(len(pdf), k))
    ...
    def connect(self, candidates, balance=1000000):
        ...
        funds = self.__rpc_interface.listfunds()

        print(sum(n["value"] for n in funds["outputs"]))

        connection_dict = self.__calculate_proposed_channel_capacities(pdf, candidates, sum(n["value"] for n in funds["outputs"]))

        print(connection_dict.items())
    ...
    def find_candidates(self, k=10):
        self.__logger.info("running the autopilot on a " +
                           "graph with {} nodes and {} edges.".format(len(self.G.nodes()), len(self.G.edges())))
...
if __name__ == '__main__':
    ...
    autopilot = Autopilot()
    connect_to = autopilot.find_candidates(4)
    # with open("data/candidates", "rb") as f:
    # candidates = pickle.load(f)
    autopilot.connect(connect_to, 1000000)
