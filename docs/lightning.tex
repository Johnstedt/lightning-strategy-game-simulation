...
\subsubsection{Commitment transaction}
...
The enforcement is possible if revocation is built into the commitment transaction, where the other party could revoke a faulty broadcast. This is possible with the addition of relative block maturity introduced with BIP68\cite{bip:0068:sequence:lock:time}. The commitment transaction has two output paths, one is the other party's funds spendable immediately. The other output has two
...
New ct created -> how invalidate works  does time lock need explaining?
