...
END UNI
BEGIN PREV
The blocks are chained through the hash of the previous block's header. A block is only valid if the hash is less than an number imposed by the difficulty target. There is no known way to reverse engineer a valid hash and to find a valid hash one has to repeatably hash the header, changing the nonce, until a sufficient hash is found. When a hash is found it can be verified by all other nodes who only need to hash it once for verification. This process is known as \textit{Proof-of-Work} generally and \textit{mining} specifically for bitcoin drawing from the similarities between the physical labor required to mint gold coins. The difficulty target adjusts every 2015th\footnote{This is a off by one bug, 2016 blocks is two weeks} block\cite{repository:bitcoin} and forces the average time for a block to be mined to always be around ten minutes. Bitcoin is not the first attempt at creating a currency based on \textit{Proof-of-Work} and differentiate from previous implementations with the difficulty adjustment\cite{back:hashcash}\footnote{Bitcoin uses the double \texttt{SHA256} hash, it is inspired by hashcash and is very similar to hashcash in the cost function is non-interactive, publicly auditable, trapdoor-free and have an unbounded probabilistic cost\cite{back:hashcash}.}.

The double-spending problem is solved with a simple consensus rule stating that all nodes agree that the longest chain of work is the valid chain and a shared transaction history. The more blocks chained after the block with a transaction, the less the chance of that transaction ever being reverted. Even if blocks are found simultaneously creating a tie, the next block will break the tie and consensus forms again.
END PREV
...
\begin{figure}[!htb]
	...
	\caption{\textit{A merkle tree resulting from a four transactions A, B, C, D.
	}}
...
The two scripts added together forms a predicate. If the predicate is true the transaction is true, Nakamoto considered naming the language predicate but went with script to be more inclusive to a broader audience\cite{nakamoto:predicate}.
...
The scripts can be made quite advanced enabling many layers on top of it, transactions is sometimes referred to as 'smart contracts'\footnote{First formally defined by Nick Szabo in 1994\cite{szabo:smart:contracts}}.
