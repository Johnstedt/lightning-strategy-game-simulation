...
The degree distribution follows from definition

\[p(k) =  \begin{cases}
& \prod_{i=1}^{k}\sum_{j=k}^{n}m\dfrac{n-j}{n^2},~\text{if}~m<k\\
& 0,~~~~~~~~~~~~~~~~~~~~~~~~~\text{otherwise}

\end{cases} \]

\[ \prod_{i=1}^{k}\sum_{j=k}^{n}m\dfrac{n-j}{n^2} \sim m(\dfrac{1}{2})^k\]
