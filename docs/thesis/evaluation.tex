...
Thus adding a age bias causes the characteristics of the degree distribution to change as seen in Figure L.
...
A more appropriate random model may be to consider $G_{n,E}$ where each time step $t$ a vertex is added to the graph and is connected with $m$ previous vertices and creates and edges when t < n. The probability of any previous vertex i being selected is thus $p_i = \dfrac{m}{t_i}$. 
As a node is not part of LN before edges are made, it reasonable to assume a node creates edges as part of joining the network.
