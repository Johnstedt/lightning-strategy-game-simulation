...
\hspace*{-0.5cm}	\max_{ x,y \in \mathbb{Z} } \sum_{\phi\in \rho} \rho(\phi) \sum_{a \neq b}
	\begin{cases}
	\Sigma_{abe}(x, y, \phi) & \Sigma_{abe}(x, y, \phi) < \sigma_{ab}(\phi)  \\
	0 & \Sigma_{abe}(x, y, \phi) > \sigma_{ab}(\phi) \\
	\dfrac{\Sigma_{abe}(x, y, \phi)}{\overline{\overline{\sigma_{ab}}} + 1} & \Sigma_{abe}(x, y, \phi) = \sigma_{ab}(\phi)  \\
	\end{cases}
\end{equation*}

where $\Sigma_{abe}(x, y, \phi)$, and $\sigma_{ab}(\phi)$ is given by

\begin{equation*}	
	\Sigma_{abe}(x, y, \phi) = \Bigg( \sum_{c \in \sigma_{ae_s},\sigma_{e_db}} c_b + c_p\phi \Bigg) +
	x + y\phi 
\end{equation*}
\begin{equation*}
...
The function over $F_B$, $F_P$ follows the same pattern as over $F$ and is shown in Figure \ref{fig:cost:function:space}.
...
As it's too cumbersome to calculate the optimal price for all edges in simulation purposes. The price range given for an edge of well connected node used as a probabilistic model in quickly setting a good fee price. Given a simulation environment with defined strategies,  
