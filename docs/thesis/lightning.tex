...
If we consider a Revocable Sequence Maturity Contract seen in figure \ref{fig:RSMC}, it is constructed by Alice and she used Bob's master public key to derive Bob's revocation public key in some prearranged manner. To invalidate the $CT$ Alice need only to disclose the private key corresponding to her revocation public key to Bob for Bob to be able to revoke a viola67tion. 
...
\section{Network spanning payments}

By combining channels into a network, a payment could potentially ripple through multiple channels and may allow parties without a direct channel transact. Such a multihop payment would need the same trustless properties for the routing middle nodes as the direct channel parties shown above.

\subsection{Hashed Timelock Contract}

Extra output in ct

produce of hash image R

Create new CT, exchanging BRT as before, settle HTLC sending keys.
\begin{figure}[hbt!]
	\centering
	\begin{lstlisting}[
	basicstyle=\small, %or \small or \footnotesize etc.
...
<@\texttt{\textcolor{black}{OP\_ENDIF}}@>
	...
	\caption{\textit{ A Hashed Timelock Contract.
	}}
	\label{fig:RSMC}
\end{figure}
...
After Alice have created $HTLC_{1}$ with $TL_{1}$, Bob is comfortable creating $HTLC_{2}$ with a $TL_{2}$ < $TL_{1}$ as the only way Carol could redeem $HTLC_{2}$ is by disclosing R to Bob. Bob in turn can then disclose R to Alice and redeem $HTLC_{1}$. If the timelocks was improperly set as $TL_{2}$ > $TL_{1}$ the $HTLC_{1}$ may be invalid by the time Carol redeem $HTLC_{2}$ leaving Bob at a loss.
